...
def main():
    ...
    agent_thread_pool = ThreadPoolExecutor(max_workers=3000)
    scheduler.start()
    agent_tasks = list()
    ...
    for j, agent_info in tasks_path.iterrows(): # iterate over each agent
        ...
        for i, task in enumerate(tasks): # iterate over each task
            if i >= args.task_num:   # by default, args.task_num is 1.
                break
    ...
    for r in as_completed(agent_tasks):
        print("*************Begin to print an attack**************")
        res = r.result()
        ...
        if attack_successful:
            print(f"Attack successful. The attacker tool is {attack_tool}; attacker goal is {attacker_goal}")
            attack_succ += 1
        else:
            print(f"Attack failed. The attacker tool is {attack_tool}; attacker goal is {attacker_goal}")

        original_successful = check_original_success(res["messages"], args, res["normal_tools"])
        if original_successful:
            orgin_succ += 1
            print(f"Original task successful. The normal tools is {res['normal_tools']}")
        else:
            print(f"Original task failed. The normal tools is {res['normal_tools']}")
